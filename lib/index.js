...
function readTokenFromProfileConfig() {
    const home = process.env.DSH_HOME ?? join(homedir(), '.dsh');
...
function saveTokenToConfigFile(token) {
    const file = pluginConfigPath();
...
export function apply(ctx, config) {
    ...
    const getModelUsage = async (start, end, granularity) => {
        ...
        if (cached && cached.expires > Date.now())
            return cached.data;
        const data = await fetchSessionModelUsageSeries(ctx.sessionPersistence, ctx.sessions, modelUsageCachePath(), start, end, granularity);
    ...
    const streamModelUsage = async (start, end, granularity, onSnapshot) => {
        return fetchSessionModelUsageSeries(ctx.sessionPersistence, ctx.sessions, modelUsageCachePath(), start, end, granularity, onSnapshot);
    };
    const disposers = [];
