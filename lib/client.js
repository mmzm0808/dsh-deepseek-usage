	...
	factory: (require) => {
		...
		*/
		...
		function DeepSeekUsageSettingsCard() {
			...
			const body = collapsed ? null : (0, react.createElement)("div", { style: bodyStyle }, (0, react.createElement)("span", { style: mutedStyle }, "登录状态：请点击下方按钮登录 DeepSeek 开放平台"), (0, react.createElement)("button", {
				style: buttonStyle,
...
`;
  ...
  `;
					...
					for (;;) {
						...
						while (newlineIndex >= 0) {
							...
							if (message.type === "snapshot" && message.series) {
								...
								renderTrends();
							} else if (message.type === "error") stateFields.trendList.innerHTML = `<div class="${NS}-trend-error">${escapeHtml(message.error ?? "加载失败")}</div>`;
						}
					}
				} catch (error) {
			...
			const renderTrendChart = (seriesList, color) => {
				...
				const sourceColors = seriesList.map((_, index) => {
					...
					return [
						...
						"#a78bfa",
						"#22d3ee"
					][index - 1] ?? `hsl(${index * 47 % 360} 70% 60%)`;
				});
      ...
      `).join("")
				...
				const legend = seriesList.length > 1 ? `<span style="display:inline-flex;gap:10px;flex-wrap:wrap;">${seriesList.map((series, index) => `
    ...
    `;
			...
			const showChartTooltip = (circle) => {
				...
				const rows = (svg ? Array.from(svg.querySelectorAll(`.${NS}-point-hit`)).filter((item) => item.getAttribute("data-x") === x) : [circle]).map((item) => {
