  ...
  `;
			...
			const render = (state) => {
				...
				if (ratio) {
					...
					stateFields.r0Total.title = ratio.has_history ? `8月17日起累计 A2/A1 = ${ratio.a2_total?.toFixed(8)} / ${ratio.a1.toFixed(8)}` : `无涨价前历史，使用默认 A1 = ${ratio.a1.toFixed(8)}`;
					stateFields.r0Today.textContent = r0Today;
					stateFields.r0Today.title = ratio.has_history ? `今日 A2/A1 = ${ratio.a2_today?.toFixed(8)} / ${ratio.a1.toFixed(8)}` : `无涨价前历史，使用默认 A1 = ${ratio.a1.toFixed(8)}`;
