...
function readStoredNumber(key: string, min: number, max: number): number | undefined {
...
export function apply(ctx: ClientContext): void {
  ...
  if (savedPanelWidth !== undefined) panel.style.width = `${savedPanelWidth}px`
  const savedBallTop = readStoredNumber('ballTop', 0, Math.max(0, window.innerHeight - ball.offsetHeight))
  if (savedBallTop !== undefined) ball.style.top = `${savedBallTop}px`
  ...
  const syncModelOptions = (models: string[]): void => {
    const seen = new Set<string>()
    const options: HTMLOptionElement[] = []
    const addOption = (model: string): void => {
  ...
  const bindTrendInteractions = (): void => {
