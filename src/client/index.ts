...
function fillModelSeries(
  series: ModelUsageSeries,
  start: string,
  end: string,
  granularity: 'hour' | 'day',
): ModelUsageSeries {
  const byTimestamp = new Map<number, ModelUsageSeries['points'][number]>()
  for (const point of series.points) byTimestamp.set(point.timestamp, point)

  const expected: Array<{ timestamp: number; label: string }> = []
  if (granularity === 'hour') {
...
export function apply(ctx: ClientContext): void {
  ...
  let trendData: ModelUsageResponse | null = null
  ...
  const switchPage = (page: 'overview' | 'trends'): void => {
    currentPage = page
    host.querySelectorAll(`.${NS}-page`).forEach(el => {
      el.classList.toggle('active', el.getAttribute('data-page') === page)
    })
    const pageSwitch = host.querySelector(`.${NS}-page-switch`) as HTMLElement
  ...
  const renderTrends = (): void => {
    ...
    stateFields.trendList.innerHTML = filled.map((series, index) => renderTrendChart(series, palette[index % palette.length])).join('')
  }
  ...
  const renderTrendChart = (series: ModelUsageSeries, color: string): string => {
    ...
    const circles = points.map((point, index) => `<circle cx="${x(index).toFixed(1)}" cy="${y(point.tokens).toFixed(1)}" r="3" fill="${color}" stroke="var(--dsu-panel-2)" stroke-width="1.5"/>`).join('')
    ...
    return `
      <div class="${NS}-chart-card">
        <div class="${NS}-chart-head">
          ...
          <span class="${NS}-chart-total">${compact(total)} Tokens</span>
        </div>
  ...
  const resizeHandle = host.querySelector('[data-action="resize"]') as HTMLElement
  let resizing = false
  let resizeStartX = 0
  ...
  const onResizeUp = (event: PointerEvent): void => {
    if (!resizing) return
    resizing = false
