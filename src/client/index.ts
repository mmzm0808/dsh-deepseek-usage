...
const CSS = `
[data-${NS}] { --dsu-bg:#0b0e14; --dsu-panel:#12161f; --dsu-panel-2:#171c27; --dsu-border:rgba(255,255,255,.08); --dsu-text:#e7ecf3; --dsu-muted:#8b95a7; --dsu-brand:#4d6bfe; --dsu-green:#34d399; --dsu-gold:#ffd166; --dsu-red:#f87171; --dsu-radius:14px; font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC","Microsoft YaHei",sans-serif; color:var(--dsu-text); }
[data-${NS}]{ position:fixed; inset:0; z-index:2147483000; pointer-events:none; }
[data-${NS}] *{ box-sizing:border-box; }
.${NS}-ball{ position:absolute; top:calc(50% - 26px); right:0; display:flex; align-items:center; gap:10px; background:linear-gradient(135deg,#1a2130,#12161f); border:1px solid rgba(77,107,254,.45); border-radius:999px 0 0 999px; padding:10px 14px 10px 12px; box-shadow:0 12px 30px rgba(0,0,0,.45),0 0 0 1px rgba(77,107,254,.12); cursor:grab; transition:box-shadow .15s ease; user-select:none; pointer-events:auto; touch-action:none; }
...
.${NS}-balance-main select{ margin-left:auto; height:30px; padding:0 8px; border-radius:8px; border:1px solid var(--dsu-border); background:var(--dsu-panel-2); color:var(--dsu-text); font:inherit; font-size:12px; }
...
export function apply(ctx: ClientContext): void {
  ...
  const ball = host.querySelector(`.${NS}-ball`) as HTMLElement
  const panel = host.querySelector(`.${NS}-panel`) as HTMLElement
  const ballValue = host.querySelector(`.${NS}-copy .v`) as HTMLElement
  const stateFields = {
    ...
    try {
      ...
      loginPollTimer = setInterval(async () => {
        try {
          ...
          const status = await statusResponse.json() as { loggedIn?: boolean; message?: string }
          if (status.loggedIn) {
            clearInterval(loginPollTimer)
  ...
  const onBallPointerDown = (event: PointerEvent): void => {
    dragMoved = false
  ...
  const onBallPointerMove = (event: PointerEvent): void => {
  ...
  const onBallPointerUp = (event: PointerEvent): void => {
    if (!ball.hasPointerCapture(event.pointerId)) return
